...
public class EngineerMonkeyTail extends Object {
    float radius;
    float offsetX, offsetY, offsetZ;

    List<List<Vector3f>> totalVertices = new ArrayList<>();
    public EngineerMonkeyTail(List<ShaderModuleData> shaderModuleDataList, List<Vector3f> vertices, Vector4f color) {
        super(shaderModuleDataList, vertices, color);
        vertices.clear();

        radius = 0.05f;
        vertices.add(new Vector3f(0.0f,0.0f,0.0f));
        vertices.add(new Vector3f(0.0f,-0.2f,-1.3f));
        vertices.add(new Vector3f(0.8f,0.3f,-0.5f));
        generate();
        setupVAOVBO();
        this.offsetX = 0.0f;
        this.offsetY = -0.2f;
