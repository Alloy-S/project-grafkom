...
public class NinjaMonkeyBeltCurve1 extends Object {
    float radius;
    float offsetX, offsetY, offsetZ;

    List<List<Vector3f>> totalVertices = new ArrayList<>();
    public NinjaMonkeyBeltCurve1(List<ShaderModuleData> shaderModuleDataList, List<Vector3f> vertices, Vector4f color) {
        super(shaderModuleDataList, vertices, color);
        vertices.clear();

        radius = 0.034f;
        vertices.add(new Vector3f(0.0f,-0.05f,0.0f));
        vertices.add(new Vector3f(-0.4f,0.2f,0.3f));
        vertices.add(new Vector3f(-0.1f,0.6f,0.4f));
        generate();
        setupVAOVBO();
        offsetX = -0.13f;
        offsetY = -0.02f;
        offsetZ = 0.12f;
        rotateObject((float) Math.toRadians(-150f), 0.0f, 0.0f, 1.0f);
